...
<c:choose>
	<c:when test="${empty prod_seq}">
		...
		<c:set var="prod_market_price" value="0"/>
	...
	<c:otherwise>
		...
		<c:set var="prod_market_price" value="${prodInfo.prod_market_price }"/>
		<c:set var="prod_price" value="${prodInfo.prod_price }"/>
		<%--<c:set var="option_cnt" value="${prodInfo.prod_price }"/> --%>
		<c:set var="option_cnt" value="${fn:length(prodOptionInfo) }"/>
	</c:otherwise>
</c:choose>

		<!-- 콘텐츠 영역 { -->
		<div id="content">
			<h1 class="bigTit">관광/체험 <c:out value="${btn_title }"/></h1>

			<!-- 상품관리 > 관광/체험 > 글쓰기 { -->
			<section id="productTravelWrite" class="page_content">
				<!-- 기본설정 -->
					...
					<table class="admin_tbl admin_write_tbl">
						...
						<tbody>
							...
							<tr>
								<th>시작가격</th>
								<td colspan="3">
									<p class="comment">* 최저가를 표시하는 노출용 가격입니다.(구매와 무관)</p>
									<input type="text" id="prod_market_price" name="prod_market_price" class="admin_input" size="10" placeholder="가격" value="<c:out value="${prod_market_price }"/>"/> 원
								</td>
							</tr>
							<tr>
								<th>기본가격</th>
								<td colspan="3">
									...
									<input type="text" id="prod_price" name="prod_price" class="admin_input" size="10" placeholder="가격" value="<c:out value="${prod_price }"/>"/> 원
								</td>
							</tr>
							<tr>
								<th>최소주문금액</th>
							...
							<tr>
								...
								<td colspan="3" id="option_list">
									...
									<p class="comment">* 옵션명과 가격이 모두 입력되어야만 저장되며, 삭제시 반드시 저장을 하여야 합니다.<br>* 기본판매가에 옵션가격이 합산됩니다.<br>* 금액추가시 10000,  금액차감시 -5000, 변동이 없을시 0 을  입력합니다. </p>
