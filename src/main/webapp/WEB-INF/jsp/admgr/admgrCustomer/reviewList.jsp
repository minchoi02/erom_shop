...
<c:import url="/Api/admgrIncludeLeftMenu.do"/>
		...
		<div id="content">
			...
			<section id="boardList" class="page_content">
				<!-- 리스트 검색 { -->
				<form method="post" id="searchForm" name="searchForm" action="/admgr/admgrCustomer/reviewList.do"  >
...
<%-- 				<nav id="tab-gnb" class="mgt20px">
					<ul class="nopm floats">
						<li <c:if test="${empty store_category }">class="active"</c:if> ><a href="/admgr/admgrCustomer/reviewList.do">전체</a></li>
						
						<li <c:if test="${store_category eq 'CD002004'}">class="active"</c:if> ><a href="/admgr/admgrCustomer/reviewList.do?store_category=CD002004&searchKey=<c:out value="${searchKey }"/>&searchKeyword=<c:out value="${searchKeyword }"/>">이롬스토어</a></li>
					</ul>
				</nav> --%>
				<!-- } 탭 버튼 -->
				...
				<table class="admin_tbl admin_list_tbl mgt10px">
					...
					<tbody>
						...
						<tr>
							...
							<td>
								<div class="starbox">
									<c:set var="prod_star" value="${bbsList.prod_star }"/>
									<c:if test="${empty prod_star }">
										<c:set var="prod_star" value="0" />
									</c:if>
									<span class="starico colorStar"><c:forEach var="i" begin="1" end="5"><i class="<c:if test="${i <= bbsList.bbs_star }">fas</c:if><c:if test="${i > bbsList.bbs_star }">far</c:if> fa-star"></i></c:forEach></span>
									<%--
									<span class="starico colorStar"><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star-half-alt"></i><i class="far fa-star"></i></span>
									 --%>
									<span class="starnum"><c:out value="${bbsList.bbs_star}"/></span>
								</div>
								<small class="colorGray">[<c:out value="${bbsList.store_nm}"/> - <c:out value="${bbsList.prod_nm}"/>]</small><br /><c:out value="${bbsList.bbs_content}"/>
							</td>
							...
							<td class="alignCenter">
								<a href="/admgr/admgrCustomer/reviewForm.do?pageIndex=<c:out value="${paginationInfo.currentPageNo }"/>&searchKey=<c:out value="${searchKey}"/>&searchKeyword=<c:out value="${searchKeyword }"/>&bbs_seq=<c:out value="${bbsList.bbs_seq }"/>" class="btns bluebtn h24px">확인</a>
