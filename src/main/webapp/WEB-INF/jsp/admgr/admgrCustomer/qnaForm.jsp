...
<c:import url="/Api/admgrIncludeLeftMenu.do"/>
		...
		<!-- 콘텐츠 영역 { -->
		<div id="content">
			<h1 class="bigTit">Q&A 내용</h1>

			<!-- 고객센터 > Q&A > 글쓰기 { -->
			<section id="boardWrite" class="page_content">
				<!-- Q&A 내용 { -->
				<form method="post" id="bbsForm" name="bbsForm" action="/admgr/admgrCustomer/	qnaAction.do" onsubmit="return qnaFormCheck()" >
					...
					<table class="admin_tbl admin_write_tbl">
						...
						<tbody>
							...
							<tr>
								...
								<input type="hidden" name="bbs_prod_id" value="<c:out value="${bbsInfo.bbs_prod_id}"/>"/>
							</tr>
							<tr>
								<th>업체</th>
				...
				<div class="btnarea mgt30px">
					<ul class="nopm floats">
						<li><a href="/admgr/admgrCustomer/qnaList.do?pageIndex=<c:out value="${pageIndex }"/>&store_category=<c:out value="${store_category }"/>&searchKey=<c:out value="${searchKey }"/>&searchKeyword=<c:out value="${searchKeyword }"/>" class="btns"><i class="fal fa-list"></i> 목록으로</a></li>
						<li class="fr">
