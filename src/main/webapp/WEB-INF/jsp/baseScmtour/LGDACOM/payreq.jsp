...
<%
 	...
 	*/
 	...
 	//String configPath = "C:/webapps/bin/payment/conf";  //LG유플러스에서 제공한 환경파일("/conf/lgdacom.conf,/conf/mall.conf") 위치 지정.
    String Domain = "http://220.123.69.229:8888";
    /*
     * 1. 기본결제 인증요청 정보 변경
     *
    ...
    /*Return URL에서 인증 결과 수신 시 셋팅될 파라미터 입니다.*/
	 ...
	 payReqMap.put("LGD_WINDOW_TYPE", "iframe");
	 payReqMap.put("LGD_CUSTOM_SWITCHINGTYPE", "IFRAME");
	 ...
	 payReqMap.put("order_id"  		 , order_id );
	 payReqMap.put("forder_total_price"  , forder_total_price );
	 payReqMap.put("forder_cart_seq"  		 , forder_cart_seq );
	 payReqMap.put("forder_user_name"  		 , forder_user_name );
	 payReqMap.put("forder_user_email"  		 , forder_user_email );
	 payReqMap.put("forder_user_hp"  		 , forder_user_hp );
	 payReqMap.put("forder_user_call"  		 , forder_user_call );
	 payReqMap.put("forder_user_zip"  		 , forder_user_zip );
	 payReqMap.put("forder_user_addr1"  		 , forder_user_addr1 );
	 payReqMap.put("forder_user_addr2"  		 , forder_user_addr2 );
	 payReqMap.put("forder_user_message"  		 , forder_user_message );
	 payReqMap.put("forder_paytype"  		 ,forder_paytype  );
	 payReqMap.put("forder_coupon_id"  		 , forder_coupon_id );
	 payReqMap.put("forder_coupon_calc_price"  		 , forder_coupon_calc_price );
	 payReqMap.put("forder_mileage"  		 , forder_mileage );
...
<table>
    ...
    <tr>
        <td colspan="2">
        LGD_BUYEREMAIL : <%= LGD_BUYEREMAIL %><br>
        CST_MID : <%= CST_MID %><br>
        LGD_MID : <%= LGD_MID %><br>
        LGD_AMOUNT : <%= LGD_AMOUNT %><br>
        LGD_TIMESTAMP : <%= LGD_TIMESTAMP %><br>
