...
<c:set var="headerName" value="bbs_notice" />
<style>
.pagenation {
	margin-top: 20px;
	text-align: center;
...
<div id="sub_visual" class="sub_visual3">
	<div class="inner">
		<div class="navigation">
			홈 <span>고객센터</span> <span class="last">공지사항</span>
		</div>
...
<div id="container">
	...
	<div class="product-list board-list">
		<div class="inner clearfix">
			...
			<h1>
				전체 <span><strong><fmt:formatNumber
							value="${TotalCnt}" type="number" /></strong>개</span>
			</h1>
			<div class="prd-sort absol noti-sort">
				<form method="get" action="" onsubmit="return bbs_search_check();">
					<article id="boardListSearch">
						<input type="text" class="search_input w100"
							placeholder="검색어를 입력해주세요" />
						<button type="submit" class="search_button">
							<i class="far fa-search"></i><span class="skip">검색</span>
						</button>
					</article>
				</form>
			</div>
			...
			<div class="bo_list_noti">
				<ul>
					<c:forEach var="bbsNoticeList" items="${bbsNoticeList}"
						varStatus="status">
						<li><a
							href="/board/view.do?bd_id=<c:out value="${bd_id}"/>&pageIndex=<c:out value="${paginationInfo.currentPageNo }"/>&bbs_seq=<c:out value="${bbsNoticeList.bbs_seq}"/>">
								<div class="txtArea">
									<div class="num">공지</div>
									<div class="subject">

										<c:out value="${bbsNoticeList.bbs_subject}" />
										<c:if test="${bbsNoticeList.comm_cnt > 0}">&nbsp;<small
												class="bbs_comment_cnt">(<fmt:formatNumber
													value="${bbsNoticeList.comm_cnt}" type="number" />)
											</small>
										</c:if>
					...
					<c:forEach var="bbsList" items="${bbsList}" varStatus="status">
						<li><a
							href="/notice_view.do?pageIndex=<c:out value="${paginationInfo.currentPageNo }"/>&searchKey=<c:out value="${searchKey}"/>&searchKeyword=<c:out value="${searchKeyword }"/>&bbs_seq=<c:out value="${bbsList.bbs_seq }"/>">
								<div class="txtArea">
									<div class="num">
										<c:out value="${StartPageNumber-status.count}" />
									</div>
									<div class="subject">

										<c:out value="${bbsList.bbs_subject}" />
										<c:if test="${bbsList.comm_cnt > 0}">&nbsp;<small
												class="bbs_comment_cnt">(<fmt:formatNumber
													value="${bbsList.comm_cnt}" type="number" />)
											</small>
										</c:if>
									...
									<div class="date">${fn:substring(reg_dt2,0,10)}
										<i class="xi-eye-o"></i> ${bbsList.bbs_hit_cnt }
									</div>
								</div>
						</a></li>
					</c:forEach>
